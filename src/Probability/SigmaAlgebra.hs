...
sigmaAlgebraDefinition = de $ do
    ...
    s ["A ", sigmaAlgebra', " or ", term salgebra, " ", m prsa, " is a ", set, " of subsets of the ", universe_, " of a ", stochasticExperiment, " with the following three properties"]
...
trivialSigmaAlgebraDefinition = de $ do
    m $ setof $ emptyset <> ", " <> univ
    " is called the "
    trivialSigmaAlgebra

measurableSpaceDefinition :: Note
measurableSpaceDefinition = de $ do
    s ["Let ", m univ, " be the ", universe, " of a ", stochasticExperiment, " and let ", m prsa, " be a ", sa]
    s ["The ", ix "tuple", " ", m (univ <> ", " <> prsa), " is called a ", measurableSpace']


trivialMeasurableSpaceDefinition :: Note
trivialMeasurableSpaceDefinition = de $ do
    m $ univ <> ", " <> setof (emptyset <> ", " <> univ)
    " is called the "
...
emptysetInSigmaAlgebra = thm $ do
    ...
    proof $ do
        s ["The first two axioms", ref sigmaAlgebraDefinitionLabel, " together with ", m (setc univ =§= emptyset), " gives the theorem"]
...
sigmaAlgebraFiniteIntersection = thm $ do
    ...
    proof $ s ["Use the infinite intersection of events in a ", sa , ref sigmaAlgebraInfiniteIntersectionLabel, " where only ", m "n", " sets ", m ai, " are not ", m univ]
...
generatedSigmaAlgebraDefinition = de $ do
    s ["Let ", m (gsa ⊆ univ), " be a ", set, " of subsets of a ", universe]
...
generatedSigmaAlgebraExists = thm $ do
    ...
    proof $ do
        ...
        ma $ saset <> "=" <> setcmpr (ss ⊆ powset univ) (ss <> text (" is a " <> sa <> " and ") <> gsa ⊆ ss)
        s [m saset, " is not empty because ", m (powset univ), " is always a ", salgebra, ".", ref discreteSigmaAlgebraDefinitionLabel]
        ...
        enumerate $ do
            item $ do
                m $ univ ∈ sb
                newline
                s ["Every ", m ss, " in ", m (powset univ), " contains ", m univ, " because they are all ", salgebra, "s"]
                s [m sb, " must therefore contain ", m univ]

            item $ do
                m $ fa ("B" ⊆ univ) $ ("B" ∈ sb) ⇒ (setc "B" ∈ sb)
                newline
                s ["Let ", m "B", " be a subset of ", m univ, " in ", m sb]
