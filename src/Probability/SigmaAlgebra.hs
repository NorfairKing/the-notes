...
sigmaAlgebraDefinitionLabel :: Label
sigmaAlgebraDefinitionLabel = delab "sigma-algebra"
...
measurableSpaceDefinition = de $ do
    ...
    s ["The ", ix "tuple", " ", m (pruniv <> ", " <> prsa), " is called a ", term "measurable spac"]
...
emptysetInSigmaAlgebra = thm $ do
    ...
    proof $ do
        s ["The first two axioms", ref sigmaAlgebraDefinitionLabel, " together with ", m (setc pruniv =§= emptyset), " gives the theorem"]
...
sigmaAlgebraFiniteIntersection = thm $ do
    ...
    proof $ s ["Use the infinite intersection of events in a ", sa , ref sigmaAlgebraInfiniteIntersectionLabel, " where only ", m "n", " sets ", m ai, " are not ", m pruniv]
...
generatedSigmaAlgebraExists = thm $ do
    ...
    proof $ do
        ...
        s [m saset, " is not empty because ", m (powset pruniv), " is always a ", salgebra, ".", ref discreteSigmaAlgebraDefinitionLabel]
