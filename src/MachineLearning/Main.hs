...
learningProblem = do
  ...
  s ["In pattern recognition, these are called ", term "features", "."]
...
linearModelAndLeastSquares = do
  ...
  s ["Here, ", m (b_ 0), " is called the ", term "intercept", or, term "bias", "."]
  ...
  s ["This is called the method of ", term "least squares", "."]
  ma $ "RSS" `funapp` beta =: sumcmpr (i =: 1) n ((pars (("y" !: i - trans ("x" !: i) * beta))) ^: 2) =: (trans . pars $ y - x * beta) * (pars $ y - x * beta)

  s ["Differentiating with respect to ", m beta, ", we get the ", term "normal equations", ":"]
